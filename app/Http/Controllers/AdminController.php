...
class AdminController extends Controller
{
    ...
    public function user()
    {
        ...
        return view('users')->with(compact('usersData'));
    }

    public function products_daily()
    {
        return view('blueprint.Daily_products');
    }

    public function products_available()
    {
        $dataOfItemsTable = DB::table('items')->paginate(6);
        return view('blueprint.Available_products', compact('dataOfItemsTable'));
    }
    public function add_user()
    {
        return view('blueprint.AddUser');
    }
    ...
    public function user_deactive(string $email)
    {
        ...
        if ($user_email) {
            $update_query_user = User::where('email', '=', $email)->where('status', '=', 'Active')->update(['status' => 'Inactive']);
    ...
    public function user_active(string $email)
    {
        $user_email = User::where('email', '=', $email)->first();
        if ($user_email) {
            $user_email = User::where('email', '=', $email)->where('status', '=', 'Inactive')->update(array('status' => 'Active'));
            session()->flash("Success", "Status Updated successfully");
            return redirect()->route('users');
        } else {
    ...
    public function user_update(Request $request)
    {
        ...
        if ($request->has('profile')) {
            ...
            $user_data->where('email', '=', $request->email)->update(array('name' => $request->name, 'mobile' => $request->mobile, 'password' => $request->password, 'address' => $request->address, 'profile' => $newFilePath));
            session()->flash("Success", "Data updated successfully");
        } else {
            $user_data->where('email', '=', $request->email)->update(array('name' => $request->name, 'mobile' => $request->mobile, 'password' => $request->password, 'address' => $request->address));
            session()->flash("Success", "Data updated successfully");
        }
        return redirect()->route('users');
    }
    ...
    public function reactive_user($email)
    {
        $user_email = User::where('email', '=', $email)->first();
        if ($user_email) {
            $user_email = User::where('email', '=', $email)->where('status', '=', 'Deleted')->update(array('status' => 'Active'));
            session()->flash("Success", "Status Updated successfully");
            return redirect()->route('users');
        } else {
            session()->flash("Error", "There is no user named : " . $user_email);
    ...
    public function delete_user($email)
    {
        $user_email = User::where('email', '=', $email)->first();
        if ($user_email) {
            $user_demo = User::where('email', '=', $email)->update(array('status' => 'Deleted'));
            if ($user_demo) {
                session()->flash("Success", "Status Updated successfully");
                return redirect()->route('users');
            }
    ...
    public function categoryShow()
    {
        ...
        return view("blueprint.category")->with(compact('category_data'));
    }
    public function categoryAdd()
    {
        return view("blueprint.category_add");
    }
    ...
    public function category_activate(string $name)
    {
        ...
        if ($category) {
            DB::table('category')->where('category_name', '=', $name)->where('category_status', '=', 'Inactive')->update(['category_status' => 'Active']);
            session()->flash("Success", "Status updated successfully");
            return redirect()->route('category');
        } else {
    ...
    public function category_deactivate(string $name)
    {
        $category = DB::table('category')->where('category_name', '=', $name)->first();
        if ($category) {
            DB::table('category')->where('category_name', '=', $name)->where('category_status', '=', 'Active')->update(['category_status' => 'Inactive']);
            session()->flash("Success", "Status updated successfully");
            return redirect()->route('category');
        } else {
            session()->flash("Error", "No Category named " . $name . " found");
    ...
    public function category_reactivate(string $name)
    {
        $category = DB::table('category')->where('category_name', '=', $name)->first();
        if ($category) {
            DB::table('category')->where('category_name', '=', $name)->where('category_status', '=', 'Deleted')->update(['category_status' => 'Active']);
            session()->flash("Success", "Status updated successfully");
            return redirect()->route('category');
        } else {
            session()->flash("Error", "No Category named " . $name . " found");
    ...
    public function category_delete(string $name)
    {
        $category = DB::table('category')->where('category_name', '=', $name)->first();
        if ($category) {
            DB::table('category')->where('category_name', '=', $name)->update(['category_status' => 'Deleted']);
            session()->flash("Success", "Status updated successfully");
            return redirect()->route('category');
        } else {
            session()->flash("Error", "No Category named " . $name . " found");
    ...
    public function category_update(Request $request)
    {
        ...
        $update =   DB::table('category')->where('id', '=', $request->category_id)->update(['category_name' => $request->category_name]);
        if ($update) {
    ...
    {
        $proQuery = DB::table('items')->where('item_name', '=', $item_name)->where('item_status', '=', 'Active')->first();
        if ($proQuery) {
            $resultOfProQuery =  DB::table('items')->where('item_name', '=', $item_name)->where('item_status', '=', 'Active')->update(array('item_status' => 'Inactive'));
    ...
    {
        $proQuery = DB::table('items')->where('item_name', '=', $item_name)->where('item_status', '=', 'Inactive')->first();
        if ($proQuery) {
            $resultOfProQuery =  DB::table('items')->where('item_name', '=', $item_name)->where('item_status', '=', 'Inactive')->update(array('item_status' => 'Active'));
            $resultOfProQuery ? session()->flash("Success", "Item Status Updated Successfully") : session()->flash("Error", "Error in Status Updating");
        } else {
    ...
    {
        $proQuery = DB::table('items')->where('item_name', '=', $item_name)->first();
        if ($proQuery) {
            $resultOfProQuery =  DB::table('items')->where('item_name', '=', $item_name)->update(array('item_status' => 'Deleted'));
            $resultOfProQuery ? session()->flash("Success", "Item Status Updated Successfully") : session()->flash("Error", "Error in Status Updating");
        } else {
    ...
    {
        $proQuery = DB::table('items')->where('item_name', '=', $item_name)->where('item_status', '=', 'Deleted')->first();
        if ($proQuery) {
            $resultOfProQuery =  DB::table('items')->where('item_name', '=', $item_name)->where('item_status', '=', 'Deleted')->update(array('item_status' => 'Active'));
            $resultOfProQuery ? session()->flash("Success", "Item Status Updated Successfully") : session()->flash("Error", "Error in Status Updating");
        } else {
    ...
    {
        $item = DB::table('items')->where('item_name', '=', $item_name)->first();
        $item_category = DB::table('category')->join('items', 'items.item_category', '<>', 'category.category_name')->where('item_name', '=', $item_name)->get();
        $item_type = DB::table('item_type')->join('items','item_type.Item_type' , '<>', 'items.item_type')->where('item_name', $item_name)->get(); 
    ...
    public function products_update(Request $request)
    {
        ...
        if ($request->has('item_image')) {
            ...
            $updatedQuery =  DB::table('items')->where('item_name', '=', $request->item_name)->update(array('item_name' => $request->item_name, 'item_price' => $request->item_price, 'item_category' => $request->item_category, 'item_image' => $fileOriginalName, 'item_type' => $request->item_type));
            if ($updatedQuery) {
            ...
            $update = DB::table('items')->where('item_name', '=', $request->item_name)->update(array('item_name' => $request->item_name, 'item_price' => $request->item_price, 'item_category' => $request->item_category, 'item_type' => $request->item_type));
            if ($update) {
                session()->flash('Success', "Data Updated successfully");
            } else {
                session()->flash('Error', "Error in Data Updation");
    ...
    public function create_services()
    {
        ...
        return view('blueprint.services', compact('service_record'));
    }
    public function add_services()
    {
        return view('blueprint.AddService');
    }
    ...
    {
        $findService = DB::table('services')->where('Service_name', '=', $service_name)->where('Service_status', '=', 'Active')->first();
        if ($findService) {
            DB::table('services')->where('Service_name', $service_name)->where('Service_status', 'Active')->update(array('Service_status' => 'Inactive'));
    ...
    {
        $serviceCheck = DB::table('services')->where('Service_name', $service_name)->where('Service_status', 'Inactive')->first();
        if ($serviceCheck) {
            DB::table('services')->where('Service_name', $service_name)->where('Service_status', 'Inactive')->update(array('Service_status' => 'Active'));
            session()->flash('Success', 'Service status updated successfully');
        }
    ...
    {
        $serviceCheck = DB::table('services')->where('Service_name', $service_name)->first();
        if ($serviceCheck) {
            DB::table('services')->where('Service_name', $service_name)->update(array('Service_status' => 'Deleted'));
            session()->flash('Success', "Service status Updated successfully");
        }
    ...
    {
        $checkServiceIsDeletedOrNot = DB::table('services')->where('Service_name', $service_name)->where('Service_status', 'Deleted')->first();
        if ($checkServiceIsDeletedOrNot) {
            DB::table('services')->where('Service_name', $service_name)->where('Service_status', 'Deleted')->update(array('Service_status' => 'Active'));
            session()->flash("Success", "Service status updated successfully");
        }
    ...
    {
        $services = DB::table('services')->where('Service_name', $service_name)->first();
    ...
    public function service_update(Request $request)
    {
        ...
        if ($request->has('service_image')) {
            ...
            $serviceUpdateQueryResult =  DB::table('services')->where('Service_name', $request->input('service_name'))->update(array('Service_price' => $request->input('service_price'), 'Service_includes' => $includes, 'Service_image' => $newFilePath));
            ...
            $serviceUpdateQueryResultWithoutFile =  DB::table('services')->where('Service_name', $request->input('service_name'))->update(array('Service_price' => $request->input('service_price'), 'Service_includes' => $includes));
    ...
    public function admin_login_validate(Request $request){
        ...
        $admin_query = DB::table('admin')->where('admin_email', $request->Email)->where('admin_password', $request->Password)->first();
        if($admin_query){
    ...
    public function service_purchased(){
        return view('blueprint.PurchasedService');
    }
}
